...
[ADR-0004](./0004-mcp-identifies-diagrams-by-internal-id.md) settled that MCP tools address Diagrams by internal id. It never covered the REST API the browser calls, because no REST contract existed yet. Scaffolding forced the question: the editor route is `/share/{shareToken}`, so the only identifier the page holds is the token.
